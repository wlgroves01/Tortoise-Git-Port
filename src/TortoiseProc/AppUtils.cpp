...
bool CAppUtils::StashSave(HWND hWnd, const CString& msg, bool showPull, bool pullShowPush, bool showMerge, const CString& mergeRev)
{
	...
	CStashSaveDlg dlg(GetParentCWnd(hWnd));
	dlg.m_sMessage = msg;
	if (dlg.DoModal() == IDOK)
	{
		CGitHash oldStash;
...
bool CAppUtils::Export(HWND hWnd, const CString* BashHash, const CTGitPath* orgPath)
{
	...
	CExportDlg dlg(GetParentCWnd(hWnd));
	if(BashHash)
...
{
	CCreateBranchTagDlg dlg(GetParentCWnd(hWnd));
	dlg.m_bIsTag = isTag;
...
{
	CCreateWorktreeDlg dlg(GetParentCWnd(hWnd));
	if (!target.IsEmpty())
...
{
	CGitSwitchDlg dlg(GetParentCWnd(hWnd));
...
{
	CIgnoreDlg ignoreDlg(GetParentCWnd(hWnd));
...
static bool Reset(HWND hWnd, const CString& resetTo, int resetType)
{
	...
	{
		CGitProgressDlg gitdlg(GetParentCWnd(hWnd));
...
{
	CResetDlg dlg(GetParentCWnd(hWnd));
...
bool CAppUtils::Pull(HWND hWnd, bool showPush, bool showStashPop)
{
	...
	CPullFetchDlg dlg(GetParentCWnd(hWnd));
	dlg.m_IsPull = TRUE;
	if (dlg.DoModal() == IDOK)
	{
...
bool CAppUtils::RebaseAfterFetch(HWND hWnd, const CString& upstream, int rebase, bool preserveMerges)
{
	while (true)
	{
		CRebaseDlg dlg(GetParentCWnd(hWnd));
		if (!upstream.IsEmpty())
...
{
	CPushDlg dlg(GetParentCWnd(hWnd));
...
{
	CRequestPullDlg dlg(GetParentCWnd(hWnd));
					...
					bool bSelectFilesForCommit)
{
	...
	{
		bFailed = false;
		CCommitDlg dlg(GetParentCWnd(hWnd));
		dlg.m_sBugID = bugid;
...
{
	CSVNDCommitDlg dcommitdlg(GetParentCWnd(hWnd));
...
BOOL CAppUtils::Merge(HWND hWnd, const CString* commit, bool showStashPop)
{
	...
	CMergeDlg dlg(GetParentCWnd(hWnd));
	if (commit)
...
{
	CMergeAbortDlg dlg(GetParentCWnd(hWnd));
	if (dlg.DoModal() == IDOK)
...
void CAppUtils::EditNote(HWND hWnd, GitRevLoglist* rev, ProjectProperties* projectProperties)
{
	...
	CInputDlg dlg(GetParentCWnd(hWnd));
...
bool CAppUtils::BisectStart(HWND hWnd, const CString& lastGood, const CString& firstBad)
{
	...
	CBisectStartDlg bisectStartDlg(GetParentCWnd(hWnd));
